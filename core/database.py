...
class Database:
    """Banco híbrido: PostgreSQL no Render e SQLite no ambiente local."""
    ...
    def sql(self, query: str) -> str:
        """Converte placeholders do SQLite para o formato do PostgreSQL."""
    ...
    def connect(self) -> Iterator[Any]:
        if self.is_postgres:
            try:
                ...
                raise RuntimeError(
                    "O PostgreSQL foi configurado, mas o pacote psycopg não está instalado."
                ) from exc
    ...
    def set_config(self, chave: str, valor: Any) -> None:
        ...
        with self.connect() as conn:
            conn.execute(query, (chave, encoded, _utc_now()))

    def get_config(self, chave: str, default: Any = None) -> Any:
        with self.connect() as conn:
            row = conn.execute(
                self.sql("SELECT valor FROM configuracoes WHERE chave=?"), (chave,)
            ).fetchone()
    ...
    def list_history(self, limit: int = 200) -> list[dict[str, Any]]:
        with self.connect() as conn:
            rows = conn.execute(
                self.sql(
                    """SELECT p.id, a.nome, a.cloud_path, p.modulo,
