...
from concurrent.futures import Future, ThreadPoolExecutor, TimeoutError as FuturesTimeoutError, as_completed, wait
...
from core.recursos_execucao import detect_cpu_capacity, recommended_profile
...
class BatchSettings:
    ...
    def from_mapping(cls, values: dict[str, Any]) -> "BatchSettings":
        ...
        cpus = detect_cpu_capacity()
        auto_tune = bool(values.get("otimizacao_automatica", True))
        profile = recommended_profile(cpus)
        ...
        return cls(
            ...
            batch_timeout_seconds=positive("timeout_lote_segundos", 420, 1800),
            auto_tune=auto_tune,
            detected_cpus=cpus,
...
def execute_parallel(
    ...
    """Executa tarefas em paralelo sem permitir bloqueio indefinido do lote."""
    if not items:
        return {}
    ...
    try:
        done, pending = wait(futures, timeout=max(1, int(timeout_seconds)))
        for future in done:
        ...
        for future in pending:
            index = futures[future]
            results[index] = (None, FuturesTimeoutError(
                f"Worker excedeu {int(timeout_seconds)}s no lote"
            ))
            future.cancel()
    finally:
...
def execute_dual_parallel(
    ...
    """Executa RREO/FNDE sem permitir que um lote bloqueie o Brasil inteiro.

    Workers que excedem o limite do lote são marcados como timeout. A escrita no
    Excel continua na thread principal e os municípios seguintes continuam sendo
    processados.
    """
    rreo_results: dict[int, tuple[R | None, Exception | None]] = {}
    fnde_results: dict[int, tuple[R | None, Exception | None]] = {}
    rreo_executor = ThreadPoolExecutor(max_workers=max(1, int(rreo_workers)))
    fnde_executor = ThreadPoolExecutor(max_workers=max(1, int(fnde_workers)))
    ...
    try:
        ...
        for future in pending:
            channel, index = futures[future]
            target = rreo_results if channel == "RREO" else fnde_results
            target[index] = (None, FuturesTimeoutError(
                f"{channel} excedeu {int(timeout_seconds)}s no lote"
            ))
            future.cancel()
    finally:
        rreo_executor.shutdown(wait=False, cancel_futures=True)
        fnde_executor.shutdown(wait=False, cancel_futures=True)
