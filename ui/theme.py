...
from core.app_recovery import ensure_recovery_watcher_started

from auth.database import AuthDatabase
from auth.session import current_user, is_admin, logout_user
from auth.views import render_switch_user


def apply_theme() -> None:
    ensure_recovery_watcher_started()
    st.markdown(
        """
        <style>
        :root { --navy:#0b1730; --blue:#2f5bea; --purple:#7c3aed; --green:#16a34a; --muted:#64748b; }
        .stApp { background:#f7f9fc; color:#12213a; font-size:16px; }
        .stApp p, .stApp li, .stApp label { font-size:14px; line-height:1.48; }
        .stApp [data-testid="stWidgetLabel"] p { font-size:14px!important; font-weight:650; }
        .stApp [data-testid="stMetricLabel"] p { font-size:13px!important; }
        .stApp [data-testid="stMetricValue"] { font-size:1.75rem!important; }
        .stApp button, .stApp a[data-testid="stLinkButton"] { font-size:14px!important; }
        ...
        /* Troca de usuário: botão amarelo e cartões */
        .st-key-sidebar_switch_user button{background:linear-gradient(90deg,#facc15,#f5b800)!important;color:#17233d!important;border:1px solid #e5a900!important;font-weight:850!important;box-shadow:0 7px 16px rgba(250,204,21,.18)!important;}
        .st-key-sidebar_switch_user button:hover{filter:brightness(1.04)!important;transform:translateY(-1px)!important;}
        .switch-section-title{font-size:10px;font-weight:900;letter-spacing:.12em;color:#6d7890;margin:4px 0 8px;}
        .switch-users-title{margin-top:16px;}
        .switch-register-title{font-size:10px;font-weight:900;letter-spacing:.12em;color:#18834a;margin:18px 0 8px;}
        .switch-current-card,.switch-selected-card{display:flex;align-items:center;gap:12px;border-radius:13px;padding:13px 14px;margin:0 0 10px;}
        .switch-current-card{background:linear-gradient(90deg,#edf3ff,#f3f0ff);border:1px solid #dbe5fb;}
        .switch-selected-card{background:linear-gradient(90deg,#fff9d7,#fff3b2);border:1px solid #f1d36a;margin-bottom:14px;}
        .switch-card-icon{width:38px;height:38px;border-radius:11px;background:#fff;display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:0 5px 14px rgba(15,23,42,.08);}
        .switch-card-name{font-size:14px;font-weight:850;color:#14213d;}
        .switch-card-profile{font-size:11px;color:#68758d;margin-top:2px;}
        div[data-testid="stDialog"] .st-key-cancel_switch_list button{margin-top:8px!important;}
        div[data-testid="stDialog"] [class*="st-key-choose_switch_user_"] button{background:linear-gradient(90deg,#fff8c9,#ffef9a)!important;color:#17233d!important;border:1px solid #edd16a!important;min-height:58px!important;text-align:left!important;justify-content:flex-start!important;font-weight:800!important;white-space:pre-line!important;box-shadow:0 5px 14px rgba(173,129,0,.10)!important;}
        div[data-testid="stDialog"] [class*="st-key-choose_switch_user_"] button:hover{background:linear-gradient(90deg,#fff4a8,#ffe77a)!important;border-color:#dcb739!important;transform:translateY(-1px)!important;}
        div[data-testid="stDialog"] [data-testid="stPageLink"] a{background:linear-gradient(90deg,#e8f8ee,#d9f2e2)!important;color:#146c3c!important;border:1px solid #b9dfc7!important;border-radius:8px!important;font-weight:800!important;min-height:42px!important;display:flex!important;align-items:center!important;justify-content:center!important;}
        </style>
        """,
...
def render_sidebar(db: AuthDatabase | None = None) -> None:
    with st.sidebar:
        st.markdown(
            """
        ...
        if user:
            st.markdown(
                f"<div class='side-label'>USUÁRIO</div>"
                f"<div class='side-status'>👤 {user['nome']}<br>"
                f"<span style='font-size:10px;color:#aab7d4!important'>{user['perfil'].title()}</span></div>",
                unsafe_allow_html=True,
            )
            if st.button("🔄 Trocar de usuário", use_container_width=True, key="sidebar_switch_user"):
                st.session_state["switch_user_open"] = True
        ...
        st.markdown(
            ...
            <div style="margin-top:34px;font-size:10px;color:#8391ae!important">VERSÃO</div><div style="font-size:12px;margin-top:5px">v1.3.7.1 RREO SAFE REGISTRY</div>
            """,
            unsafe_allow_html=True,
        )

    # O estado mantém a janela aberta durante a escolha do usuário e a digitação da senha.
    if st.session_state.get("switch_user_open"):
        render_switch_user(db or AuthDatabase())
