...
class AuthDatabase:
    ...
    def list_active_users(self) -> list[dict[str, Any]]:
        """Lista somente usuários ativos, usados na troca rápida de usuário."""
        order_expression = "LOWER(nome)" if self.db.is_postgres else "nome COLLATE NOCASE"
        with self.db.connect() as conn:
            rows = conn.execute(
                f"""SELECT id, nome, email, perfil, ativo, trocar_senha, criado_em, ultimo_login
                FROM usuarios WHERE ativo=1 ORDER BY {order_expression}"""
            ).fetchall()
        return [dict(row) for row in rows]

    def delete_user(self, user_id: int) -> None:
        """Exclui um usuário não administrador e seus códigos de recuperação."""
        with self.db.connect() as conn:
            row = conn.execute(
                f"SELECT perfil FROM usuarios WHERE id={self._p}",
                (user_id,),
            ).fetchone()
            if row is None:
                raise ValueError("Usuário não encontrado.")
            if row["perfil"] == "administrador":
                raise ValueError("A conta de administrador não pode ser excluída.")
            conn.execute(
                f"DELETE FROM recuperacao_senha WHERE usuario_id={self._p}",
                (user_id,),
            )
            conn.execute(
                f"DELETE FROM usuarios WHERE id={self._p}",
                (user_id,),
            )
