...
class AuthDatabase:
    ...
    def create_user(
        ...
        try:
            ...
            is_duplicate = isinstance(exc, sqlite3.IntegrityError) or exc.__class__.__name__ in {
                "UniqueViolation",
                "IntegrityError",
            }
            if is_duplicate:
    ...
    def authenticate(self, email: str, password: str) -> dict[str, Any] | None:
        ...
        query = (
            ...
            if self.db.is_postgres
            else "SELECT * FROM usuarios WHERE email=? COLLATE NOCASE AND ativo=1"
        )
        with self.db.connect() as conn:
            ...
            conn.execute(
                self.db.sql("UPDATE usuarios SET ultimo_login=? WHERE id=?"),
                (now, row["id"]),
            )
            data = dict(row)
    ...
    def get_user_by_email(self, email: str) -> dict[str, Any] | None:
        ...
        query = (
            ...
            if self.db.is_postgres
            else "SELECT * FROM usuarios WHERE email=? COLLATE NOCASE"
        )
        with self.db.connect() as conn:
            row = conn.execute(query, (normalized_email,)).fetchone()
    ...
    def list_users(self) -> list[dict[str, Any]]:
        order = "LOWER(nome)" if self.db.is_postgres else "nome COLLATE NOCASE"
        with self.db.connect() as conn:
            rows = conn.execute(
                f"""SELECT id, nome, email, perfil, ativo, trocar_senha, criado_em, ultimo_login
                FROM usuarios ORDER BY {order}"""
            ).fetchall()
    ...
    def set_active(self, user_id: int, active: bool) -> None:
        with self.db.connect() as conn:
            conn.execute(
                self.db.sql("UPDATE usuarios SET ativo=? WHERE id=?"),
                (int(active), user_id),
            )
    ...
    def update_password(self, user_id: int, password: str, force_change: bool = False) -> None:
        ...
        with self.db.connect() as conn:
            conn.execute(
                self.db.sql("UPDATE usuarios SET senha_hash=?, trocar_senha=? WHERE id=?"),
    ...
    def verify_recovery(self, email: str, code: str) -> dict[str, Any] | None:
        ...
        with self.db.connect() as conn:
            ...
            if row["codigo_hash"] != hash_recovery_code(code.strip()):
                return None
            conn.execute(
                self.db.sql("UPDATE recuperacao_senha SET usado_em=? WHERE id=?"),
