...
@st.dialog("Primeiro acesso", width="large")
def _first_admin_dialog(db: AuthDatabase) -> None:
    _auth_header(
        "🔐",
        "Crie o primeiro administrador",
        "Configuração inicial realizada apenas uma vez",
    )
    st.markdown(
        """
        <div class="auth-welcome">
            Bem-vindo ao <strong>Extrator RREO Cloud</strong>.<br>
            Para liberar o sistema, cadastre a conta principal de administração.
        </div>
        """,
        unsafe_allow_html=True,
    )

    nome = st.text_input("Nome completo", key="first_admin_name")
    email = st.text_input("E-mail", key="first_admin_email")
    senha = st.text_input("Senha", type="password", key="first_admin_password")
    confirmar = st.text_input("Confirmar senha", type="password", key="first_admin_confirm")
