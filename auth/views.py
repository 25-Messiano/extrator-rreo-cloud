...
from .session import current_user, login_user
...
def _switch_user_dialog(db: AuthDatabase) -> None:
    _auth_header("🔄", "Trocar de usuário", "Escolha o usuário e informe apenas a senha")

    current = current_user() or {}
    users = [u for u in db.list_active_users() if u["id"] != current.get("id")]
    if not users:
        st.info("Não há outro usuário ativo cadastrado.")
        st.caption("O administrador pode criar um usuário na página Usuários.")
        return

    labels = {f"{u['nome']} — {u['perfil'].title()}": u for u in users}
    selected_label = st.selectbox("Usuário", list(labels), key="switch_user_selected")
    selected = labels[selected_label]

    with st.form("switch_user_form", clear_on_submit=False):
        st.text_input("E-mail", value=selected["email"], disabled=True)
        password = st.text_input(
            "Senha",
            type="password",
            key="switch_user_password",
    ...
    if submitted:
        user = db.authenticate(selected["email"], password)
        if not user:
            st.error("Senha inválida ou usuário bloqueado.")
            return
        login_user(user)
        st.session_state.pop("switch_user_password", None)
