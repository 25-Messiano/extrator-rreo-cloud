...
def list_fnde_state_folders() -> list[str]:
    """Lista pastas no padrão '<Estado> FNDE - <UF>_<ANO>'."""
    import re
    client = get_storage_client()
    iterator = client.list_blobs(BUCKET_NAME, prefix=FNDE_ROOT_PREFIX, delimiter="/")
    list(iterator)
...
def list_fnde_pdfs(folder: str) -> list[dict[str, Any]]:
    client = get_storage_client()
    prefix = f"{FNDE_ROOT_PREFIX}{folder}/"
    files: list[dict[str, Any]] = []
...
def find_fnde_folder(uf: str, year: int | str | None = None) -> str | None:
    import re
    target = str(uf or "").upper()
    year_text = str(year) if year else None
    for folder in list_fnde_state_folders():
        match = re.search(r"-\s*([A-Z]{2})_(\d{4})$", folder, re.IGNORECASE)
        if not match:
            continue
        if match.group(1).upper() == target and (not year_text or match.group(2) == year_text):
            return folder
    return None
