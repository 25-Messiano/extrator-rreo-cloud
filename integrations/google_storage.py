...
def _uf_from_state_folder(folder: str) -> str:
    ...
    match = re.search(r"(?:^|_)([A-Z]{2})$", str(folder or "").strip(), re.IGNORECASE)
...
def list_pdfs(state: str, year: int | str = 2025) -> list[dict[str, Any]]:
    """Lista PDFs RREO da UF/pasta informada na estrutura nova."""
    uf = _uf_from_state_folder(state) or str(state or "").upper().strip()
    folder = find_rreo_folder(uf, year)
    if folder:
    ...
    # Compatibilidade com o nome de pasta antigo.
...
def describe_state_files(uf: str, year: int | str = 2025) -> dict[str, Any]:
    ...
    return {
        ...
        "rreo": list_pdfs(rreo_folder or target, year) if rreo_folder else [],
        "fnde": list_fnde_pdfs(fnde_folder, year) if fnde_folder else [],
    }
