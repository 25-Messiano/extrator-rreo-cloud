...
RREO_SOURCE_BASE_PREFIX = os.getenv(
    ...
    "01_Arquivo_dos_Estados_RREO_e_FNDE/4_APPDOWELEVER/RREO",
).strip().strip("/") + "/"
...
# Estrutura antiga mantida apenas para FNDE enquanto a nova origem FNDE ainda nao existe.
# RREO NAO usa fallback legado: a unica origem autorizada e APPDOWELEVER/RREO.
...
def list_appdowelever_rreo_state_folders(year: int | str = 2025) -> list[str]:
    """Lista as pastas estaduais somente na fonte oficial APPDOWELEVER/RREO."""
    if not RREO_SOURCE_ENABLED:
        raise StorageConfigurationError(
            "Fonte RREO APPDOWELEVER desativada. Defina RREO_SOURCE_ENABLED=true."
        )
...
def list_rreo_state_folders(year: int | str = 2025) -> list[str]:
    """RREO: lista exclusivamente a estrutura oficial do APPDOWELEVER."""
    return list_appdowelever_rreo_state_folders(year)
...
def rreo_source_location(
    uf: str,
    year: int | str = 2025,
    bimestre: int | str | None = None,
) -> dict[str, str | None]:
    """Retorna a origem oficial exata usada para RREO.

    Estrutura: bucket/01_.../4_APPDOWELEVER/RREO/{ano}/{estado}/B{n}/
    """
    target = str(uf or "").upper().strip()
    folder = find_appdowelever_rreo_folder(target, year)
    bim = _normalized_bimester(bimestre)
    prefix = (
        f"{_appdowelever_year_prefix(year)}{folder}/{bim}/"
        if folder else None
    )
    return {
        "bucket": RREO_SOURCE_BUCKET,
        "prefix": prefix,
        "uf": target,
        "folder": folder,
        "bimestre": bim,
...
def list_appdowelever_rreo_pdfs_by_uf(
    ...
    """Lê RREO exclusivamente do bucket oficial do APPDOWELEVER."""
    if not RREO_SOURCE_ENABLED:
        raise StorageConfigurationError(
            "Fonte RREO APPDOWELEVER desativada. Defina RREO_SOURCE_ENABLED=true."
        )
    location = rreo_source_location(uf, year, bimestre)
    prefix = location.get("prefix")
    if not prefix:
        return []
    files = _list_pdfs_under_prefix_from_bucket(RREO_SOURCE_BUCKET, str(prefix))
    for item in files:
        item["source"] = "APPDOWELEVER"
        item["source_bucket"] = RREO_SOURCE_BUCKET
        item["source_prefix"] = str(prefix)
    return files


def list_rreo_pdfs_by_uf(uf: str, year: int | str = 2025, bimestre: int | str | None = None) -> list[dict[str, Any]]:
    """Lista RREO exclusivamente na fonte oficial APPDOWELEVER.

    Nao existe fallback para bucket/prefixo antigo. Ausencia na fonte oficial
    retorna lista vazia; falhas de acesso/configuracao sao propagadas ao painel.
    """
    return list_appdowelever_rreo_pdfs_by_uf(uf, year, bimestre)


def list_pdfs(state: str, year: int | str = 2025) -> list[dict[str, Any]]:
    """Compatibilidade de assinatura, sempre roteando para APPDOWELEVER/RREO."""
...
def health_check() -> dict[str, Any]:
    try:
        client = get_storage_client()
        result_bucket = client.get_bucket(BUCKET_NAME)
        source_bucket = client.get_bucket(RREO_SOURCE_BUCKET) if RREO_SOURCE_ENABLED else None
        return {
            "ok": True,
            "bucket": result_bucket.name,
            "rreo_source_bucket": source_bucket.name if source_bucket else None,
            "rreo_source_base_prefix": RREO_SOURCE_BASE_PREFIX,
        }
