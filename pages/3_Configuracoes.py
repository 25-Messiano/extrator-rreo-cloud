...
from core.processamento_lotes import BatchSettings
...
with st.form("config_form"):
    ...
    system["otimizacao_automatica"] = st.checkbox(
        "Otimizar automaticamente conforme CPUs disponíveis",
        value=system.get("otimizacao_automatica", True),
        help=(
            "Recomendado. Detecta a capacidade da instância do Render e divide "
            "automaticamente RREO, FNDE/OCR, Gemini e tamanho do lote."
        ),
    )
    runtime = BatchSettings.from_mapping(system)
    st.info(
    ...
    system["gemini_concorrencia"] = n1.number_input(
        ...
        help="No modo automático este valor é ajustado conforme as CPUs disponíveis.",
    )
    ...
    system["timeout_lote_segundos"] = st.number_input(
        "Tempo máximo por lote (segundos)", min_value=60, max_value=1800,
        value=int(system.get("timeout_lote_segundos", 420)), step=30,
        help="Ao exceder este tempo, o lote registra timeout e o processamento continua.",
    )
    ...
    st.caption(
        "Perfis automáticos: 1 CPU = 6/2/1/1; 2 CPUs = 10/4/2/1; "
        "4 CPUs = 12/6/3/2 (lote/RREO/FNDE/Gemini). "
        "Desative a otimização automática somente se quiser definir esses valores manualmente."
    )
...
st.info("Identificação unificada: IBGE de 7 dígitos tem prioridade; sem IBGE, RREO/FNDE usam município normalizado + UF e similaridade alta sem aceitar ambiguidades. A leitura interna do RREO continua sendo apenas auditoria.")
