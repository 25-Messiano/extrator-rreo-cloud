...
def baixar_planilha_origem(
    ...
    uf: str,
    destination: Path,
    codigos_necessarios: set[str],
) -> tuple[Path, str]:
    """Seleciona uma planilha que realmente contenha os códigos solicitados.

    Para município único, prioriza arquivo RREO com o mesmo código IBGE.
    Para lote estadual, evita usar uma planilha municipal aleatória e usa uma
    consolidada compatível ou a planilha-base.
    """
    todos = list_results()
    codigos = {normalizar_codigo_ibge(c) for c in codigos_necessarios if c}

    candidatos: list[dict[str, Any]] = []
    if len(codigos) == 1:
        codigo = next(iter(codigos))
        candidatos.extend(x for x in todos if codigo in x["name"])

    # Depois, resultados relacionados ao estado/UF, mas arquivos municipais de
    # outro código ficam excluídos quando só um município foi solicitado.
    chaves = [normalizar_texto(pasta_rreo_correspondente(state)), normalizar_texto(uf)]
    for item in todos:
        nome_norm = normalizar_texto(item["name"])
        codigos_nome = set(re.findall(r"(?<!\d)\d{7}(?!\d)", item["name"]))
        if len(codigos) == 1 and codigos_nome and not (codigos_nome & codigos):
            continue
        if any(chave and chave in nome_norm for chave in chaves):
            candidatos.append(item)

    # Mantém ordem e elimina repetições.
    vistos: set[str] = set()
    candidatos = [x for x in candidatos if not (x["blob_name"] in vistos or vistos.add(x["blob_name"]))]

    for indice, candidato in enumerate(candidatos[:40], start=1):
        temporario = destination.with_name(f"candidato_{indice}.xlsx")
        try:
            download_file(candidato["blob_name"], temporario)
            presentes = _codigos_na_planilha(temporario)
            if codigos.issubset(presentes):
                temporario.replace(destination)
                return destination, f"Resultado RREO compatível: {candidato['name']}"
        except Exception:
            pass
        finally:
    ...
    if not PLANILHA_BASE.exists():
        raise FileNotFoundError(
            "Nenhuma planilha RREO compatível foi localizada e a planilha-base "
            "local não foi encontrada em data/."
        )

    destination.write_bytes(PLANILHA_BASE.read_bytes())
    presentes = _codigos_na_planilha(destination)
    faltantes = sorted(codigos - presentes)
    if faltantes:
        raise RuntimeError(
            "A planilha-base não contém os códigos IBGE necessários: "
            + ", ".join(faltantes)
        )
    return destination, "Planilha-base local (nenhum RREO compatível encontrado)"
...
state = st.selectbox("Pasta estadual FNDE", states)
...
if not files:
    st.warning(f"Nenhum PDF FNDE encontrado em {state}.")
    st.stop()

selected_files = files
if mode == "Municipio unico":
...
if start:
    ...
    uf = uf or extrair_uf(state) or state
    ...
    with tempfile.TemporaryDirectory(prefix="fnde_cloud_") as temp_dir:
        ...
        try:
            codigos_selecionados = {
                ...
                if (match := re.search(r"(?<!\d)(\d{7})(?!\d)", item["name"]))
            }
                ...
                for index, item in enumerate(selected_files, start=1):
                    ...
                    try:
                        ...
                        logs.append(
                            ...
                            f"PNAE={values['PNAE']:.2f}; "
                            f"PNATE={values['PNATE']:.2f}; "
                            f"PDDE={values['PDDE']:.2f}; "
                            f"QSE={values['QSE']:.2f}"
                        )
            ...
            uploaded = upload_resultado_fnde(output_xlsx, state)
