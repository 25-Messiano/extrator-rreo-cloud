...
def _codigo_ibge_nome_arquivo(nome: str) -> str:
    match = re.search(r"(?<!\d)(\d{7})(?!\d)", nome)
...
def baixar_planilha_origem(
    ...
    """Evita usar a planilha municipal de outro município como origem."""
    codigo_alvo = ""
    if len(selected_files) == 1:
        codigo_alvo = _codigo_ibge_nome_arquivo(selected_files[0]["name"])

    search_keys = [pasta_rreo_correspondente(state), uf, state]
    ...
    for key in search_keys:
        ...
        if not key or key in used_keys:
            continue
        used_keys.add(key)
        candidates.extend(list_results(key))

    # Município único: só aceita resultado RREO com o mesmo código IBGE.
    if codigo_alvo:
        exatos = [item for item in candidates if codigo_alvo in item["name"]]
        if exatos:
            selected = exatos[0]
            download_file(selected["blob_name"], destination)
            return destination, f"Resultado RREO do mesmo município: {selected['name']}"

    # Estado inteiro: aceita apenas arquivo consolidado, sem código municipal no nome.
    consolidados = [
        item for item in candidates
        if not _codigo_ibge_nome_arquivo(item["name"])
        or any(p in normalizar_texto(item["name"]) for p in ("GERAL", "CONSOLIDADO", "ESTADO"))
    ]
    if len(selected_files) > 1 and consolidados:
        selected = consolidados[0]
        download_file(selected["blob_name"], destination)
        return destination, f"Resultado RREO consolidado: {selected['name']}"

    if not PLANILHA_BASE.exists():
        raise FileNotFoundError("A planilha-base local não foi encontrada em data/.")
    destination.write_bytes(PLANILHA_BASE.read_bytes())
    detalhe = f"; não foi encontrada planilha RREO do código {codigo_alvo}" if codigo_alvo else ""
    return destination, f"Planilha-base local{detalhe}"
