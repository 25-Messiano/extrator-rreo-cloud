...
selection_note = "Todos os PDFs da seleção estadual."

if all_states:
    mode = "Estado inteiro"
    selection_note = "Todos os PDFs de todas as pastas estaduais FNDE."
...
elif mode == "Municipios selecionados":
    selected_names = st.multiselect(
        "Escolha os municípios/PDFs",
        options=[item["name"] for item in files],
        default=[],
        help="Você pode pesquisar pelo nome e marcar quantos municípios desejar.",
    )
    selected_set = set(selected_names)
    selected_files = [item for item in files if item["name"] in selected_set]
    selection_note = f"{len(selected_files)} município(s) selecionado(s) manualmente."
elif mode == "Amostra":
    sample_cols = st.columns([1.0, 1.4])
    with sample_cols[0]:
        sample_size = st.number_input(
            "Quantidade da amostra",
            min_value=1,
            max_value=max(1, min(len(files), 100)),
            value=min(5, max(1, len(files))),
            step=1,
        )
    with sample_cols[1]:
    ...
    quantity = min(int(sample_size), len(files))
    if sample_rule == "Aleatórios (repetível)":
        seed = f"{state}|{year}|FNDE"
        selected_files = random.Random(seed).sample(files, quantity)
        selected_files = sorted(selected_files, key=lambda item: normalizar_texto(item["name"]))
    else:
        selected_files = files[:quantity]
    selection_note = f"Amostra de {len(selected_files)} município(s): {sample_rule}."

if not selected_files:
    st.warning("Selecione pelo menos um município/PDF antes de processar.")
...
with middle:
    ...
    st.caption(
        f"Pasta: {'TODOS OS ESTADOS' if all_states else state} | "
        f"Modo: {mode} | PDFs selecionados: {len(selected_files)} | "
        f"Gemini Vision: {'ativo' if gemini_ok else 'inativo'}"
    )
    start = st.button(
