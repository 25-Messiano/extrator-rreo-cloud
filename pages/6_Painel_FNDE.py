...
def listar_pdfs_fnde(state: str) -> list[dict[str, Any]]:
    client = get_storage_client()
    prefix = f"{FNDE_PDFS_PREFIX}{state}/"
...
def baixar_planilha_origem(
    ...
    candidates = list_results(state)

    if not candidates and uf and uf != state:
        candidates = list_results(uf)
    ...
    if not PLANILHA_BASE.exists():
        raise FileNotFoundError(
            "A planilha-base local nao foi encontrada em data/."
        )
...
if not states:
    st.warning(
        "Nenhuma pasta estadual do FNDE foi encontrada. "
        "Use a estrutura ARQUIVO_DE_ESTADOS_FNDE/"
        "PDF - DOS MUNICIPIOS/<ESTADO>/."
    )
    st.stop()
...
with c3:
    year = st.selectbox("Ano", [2025, 2024, 2023], index=0)
...
if start:
    ...
    with tempfile.TemporaryDirectory(prefix="fnde_cloud_") as temp_dir:
        ...
        try:
            ...
            try:
                for index, item in enumerate(selected_files, start=1):
                    ...
                    try:
                        ...
                        logs.append(f"ERRO {item['name']}: {error}")
            ...
            job["status"] = "Falha"
            logs.append(f"ERRO GERAL: {error}")
            log_box.code("\n".join(logs[-18:]), language=None)
