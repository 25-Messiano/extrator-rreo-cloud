...
if executar:
    ...
    try:
        if todos_os_estados:
            state.update({"status":"Em andamento","progress":0,"current":"Preparando todos os estados...","success":0,"errors":0,"total":0})
            divergencias=[]
            progress_slot=st.progress(0,text="Preparando todos os estados...")
            ...
            for estado_item in estados_cloud:
                ...
                if not uf_item:
                    logs.append(f"{datetime.now().strftime('%H:%M:%S')}  UF não identificada: {estado_item}")
                    continue

                arquivos_item=carregar_pdfs_estado(estado_item)
                workbook_consulta=load_workbook(PLANILHA_BASE,read_only=False,data_only=False)
                worksheet_consulta=escolher_aba_principal(workbook_consulta)
                municipios_item=carregar_municipios(worksheet_consulta,uf_item)
                workbook_consulta.close()

                trabalhos.append((estado_item,uf_item,arquivos_item,municipios_item))
                state["total"]+=len(arquivos_item)

            total_geral=state["total"]
            processados_geral=0
            arquivos_gerados=[]
            caminhos_cloud=[]

            for estado_item,uf_item,arquivos_item,municipios_item in trabalhos:
                caminho_saida=pasta_temporaria/gerar_nome_saida(uf_item,"Estado inteiro")
                ...
                logs.append(f"{datetime.now().strftime('%H:%M:%S')}  Iniciando estado {uf_item}")
                logbox.code("\n".join(logs[-12:]),language=None)

                for arquivo_pdf in arquivos_item:
                    processados_geral+=1
                    state["current"]=f"{uf_item} - {arquivo_pdf['name']}"
                    logs.append(f"{datetime.now().strftime('%H:%M:%S')}  Processando {uf_item} - {arquivo_pdf['name']}")
                    logbox.code("\n".join(logs[-12:]),language=None)
                    status_slot.info(f"Processando {processados_geral} de {total_geral}: {uf_item} - {arquivo_pdf['name']}")

                    try:
                        resultados,texto_pdf=processar_um_pdf(arquivo_pdf,pasta_temporaria)
                        municipio_encontrado,nota,origem=localizar_municipio_pdf(arquivo_pdf,texto_pdf,municipios_item)

                        if municipio_encontrado is None:
                            state["errors"]+=1
                            divergencias.append({"UF":uf_item,"arquivo":arquivo_pdf["name"],"problema":"Município não identificado","nota":nota})
                        else:
                            preencher_resultados(worksheet,municipio_encontrado["row"],resultados,colunas_codigos)
                            state["success"]+=1
                    except Exception as error:
                        state["errors"]+=1
                        divergencias.append({"UF":uf_item,"arquivo":arquivo_pdf["name"],"problema":str(error),"nota":0})

                    state["progress"]=(processados_geral/total_geral) if total_geral else 1.0
                    progress_slot.progress(state["progress"],text=f"Progresso geral: {state['progress']*100:.0f}%")

                workbook.save(caminho_saida)
                workbook.close()
                resultado_cloud=upload_result(caminho_saida,uf_item)
                arquivos_gerados.append(caminho_saida)
                caminhos_cloud.append(resultado_cloud["blob_name"])

            agora=datetime.now().strftime("%Y%m%d_%H%M%S")
            nome_zip=f"RREO_TODOS_OS_ESTADOS_{agora}.zip"
            memoria_zip=io.BytesIO()
            with zipfile.ZipFile(memoria_zip,"w",compression=zipfile.ZIP_DEFLATED) as pacote:
                for arquivo_excel in arquivos_gerados:
                    pacote.write(arquivo_excel,arcname=arquivo_excel.name)

            st.session_state["last_result"]={
                "name":nome_zip,
                "bytes":memoria_zip.getvalue(),
                "cloud":f"{len(caminhos_cloud)} planilha(s) salva(s) no Cloud Storage.",
                "mime":"application/zip",
            }
            state["status"]="Concluído"
            state["current"]="Finalizado"
            state["progress"]=1.0
            logs.append(f"{datetime.now().strftime('%H:%M:%S')}  Extração de todos os estados concluída: {state['success']} sucesso(s), {state['errors']} erro(s).")
            status_slot.success("Processamento de todos os estados concluído. As planilhas foram salvas no Cloud Storage.")
