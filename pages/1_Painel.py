...
from core.auditoria import append_log, timestamp, write_audit, write_missing
...
GERAR_LOG = bool(SISTEMA_CONFIG.get("gerar_log_processamento", True))
                ...
                try:
                    ...
                    if municipio_encontrado is None:
                        ...
                        if GERAR_LOG:
                            append_log(workbook, {
                                "Data/Hora": timestamp(),
                                "PDF processado": arquivo_pdf["name"],
                                ...
                                "Campos preenchidos": filled,
                                "Códigos encontrados": ", ".join(found_codes),
                                "Códigos ausentes": ", ".join(missing_codes),
                                "FNDE processado": "SIM" if fnde_count else "NÃO",
                                "Programas FNDE encontrados": ", ".join(k for k,v in fnde_values.items() if v is not None),
                                "Status": "OK" if filled else "PARCIAL",
                                "Observações": "; ".join(fnde_warnings),
                            })
                        state["success"] += 1
                    ...
                    if GERAR_LOG:
                        append_log(workbook, {
                            "Data/Hora": timestamp(), "PDF processado": arquivo_pdf["name"],
            ...
            for trabalho in trabalhos:
                for municipio in trabalho["municipios"]:
                    ...
                    missing_rows.append({
                        ...
                        "PDF correspondente": arquivo["name"] if arquivo else "",
