...
from integrations.gemini import identify_rreo_municipality
...
def identificar_municipio_rreo_no_conteudo(
    ...
    """
    Regra oficial do RREO: o município do conteúdo interno prevalece sobre
    o nome do arquivo. O nome externo nunca é usado para decidir a linha final.
    """
    texto = normalizar_texto(texto_pdf[:30000])
    if not texto:
        return None, 0.0, "conteúdo vazio"

    uf_norm = normalizar_texto(uf_esperada)

    # Primeiro procura nomes completos do cadastro do estado dentro do PDF.
    ...
    for municipio in municipios:
        ...
        pos = texto.find(nome)
        if pos >= 0:
            encontrados.append((pos, municipio))

    if encontrados:
        # Cabeçalhos normalmente aparecem nas primeiras linhas/páginas.
        encontrados.sort(key=lambda item: item[0])
        pos, municipio = encontrados[0]
        contexto = texto[max(0, pos - 180): pos + len(municipio["nome_normalizado"]) + 180]
        confianca = 1.0 if uf_norm in contexto or f"/{uf_norm}" in contexto else 0.96
        return municipio, confianca, "conteúdo interno do PDF"

    # Segunda via: procura padrões explícitos de ente/município no cabeçalho.
    padroes = [
        r"(?:MUNICIPIO|MUNICÍPIO|ENTE\s+FEDERADO|PREFEITURA\s+MUNICIPAL\s+DE)\s*[:\-]?\s*([A-Z0-9 .'-]{3,80})",
        r"RELATORIO\s+RESUMIDO.*?([A-Z][A-Z0-9 .'-]{3,60})\s*[-/]\s*" + re.escape(uf_norm),
    ]
    candidatos: list[str] = []
    for padrao in padroes:
        for match in re.finditer(padrao, texto, flags=re.S):
            candidato = match.group(1).strip(" -/|:")
            candidato = re.split(r"\s{2,}|\n|EXERCICIO|PERIODO|CNPJ", candidato)[0].strip()
            if candidato:
    ...
    for candidato in candidatos:
        municipio, nota = localizar_municipio_por_nome(candidato, municipios, limite_minimo=0.78)
        if municipio and nota > melhor_nota:
            melhor = municipio
            melhor_nota = nota

    if melhor:
        return melhor, melhor_nota, "cabeçalho interno aproximado"

    # Terceira via: Gemini lê somente o conteúdo interno e escolhe um nome
    # da lista oficial do estado. O código IBGE continua vindo exclusivamente
    # da planilha-base após a correspondência Município + UF.
    try:
        nome_gemini, modelo, tentativas = identify_rreo_municipality(
            texto_pdf=texto_pdf,
            uf_esperada=uf_esperada,
            municipios_oficiais=[item["nome"] for item in municipios],
        )
        if nome_gemini:
            municipio, nota = localizar_municipio_por_nome(
                nome_gemini,
                municipios,
                limite_minimo=0.90,
            )
            if municipio:
                return (
                    municipio,
                    max(nota, 0.90),
                    f"Gemini conteúdo interno ({modelo}; {tentativas} tentativa(s))",
                )
    except Exception:
        pass

    return None, 0.0, "município interno não identificado com segurança"
...
def construir_indice_rreo_interno(
    arquivos_pdf: list[dict[str, Any]],
    municipios: list[dict[str, Any]],
    uf: str,
    temp_root: Path,
    ...
    """Lê cada RREO e cria índice pelo município INTERNO + UF.

    O nome externo nunca decide a associação. Depois de identificar o município
    interno, o código IBGE é obtido do cadastro carregado da planilha-base.
    """
    indice: dict[str, dict[str, Any]] = {}
    falhas: list[dict[str, str]] = []
    pasta = temp_root / "indice_rreo" / uf
    pasta.mkdir(parents=True, exist_ok=True)

    def worker(arquivo: dict[str, Any]) -> dict[str, Any]:
        caminho = pasta / arquivo["name"]
        download_pdf(arquivo["blob_name"], caminho)
        try:
            valores, texto = processar_rreo(caminho, CODIGOS_RREO)
            municipio, confianca, origem = identificar_municipio_rreo_no_conteudo(
                texto, municipios, uf
            )
            return {
                "arquivo": arquivo,
                "values": valores,
                "text": texto,
                "municipio_interno": municipio,
                "confianca_municipio": confianca,
                "origem_municipio": origem,
                "error": "" if municipio else origem,
            }
        finally:
            caminho.unlink(missing_ok=True)

    with ThreadPoolExecutor(max_workers=max(1, min(int(workers), 6))) as executor:
        futuros = {executor.submit(worker, arquivo): arquivo for arquivo in arquivos_pdf}
        for futuro in as_completed(futuros):
            ...
            municipio = item.get("municipio_interno")
            if not municipio:
                falhas.append({
                    "arquivo": arquivo.get("name", ""),
                    "erro": item.get("error", "Município interno não identificado"),
                })
                continue

            codigo = municipio["codigo_ibge"]
            existente = indice.get(codigo)
            if existente is None or float(item.get("confianca_municipio", 0)) > float(existente.get("confianca_municipio", 0)):
                indice[codigo] = item
            else:
                falhas.append({
                    "arquivo": arquivo.get("name", ""),
                    "erro": f"Duplicidade interna para {municipio['nome']}/{uf}; mantido arquivo de maior confiança.",
                })
...
def _indice_fnde_por_ibge(uf: str, year: int) -> dict[str, dict[str, Any]]:
    ...
    return {
        code: item
        for item in list_fnde_pdfs(folder, year)
...
def _rreo_worker_payload(payload: dict[str, Any], temp_root: Path) -> dict[str, Any]:
    pre_processado = payload.get("rreo_preprocessado")
    if pre_processado:
        return pre_processado
    ...
    municipio_interno, confianca, origem = identificar_municipio_rreo_no_conteudo(
        text,
        payload["trabalho"]["municipios"],
        payload["uf"],
    )
    return {
        "values": values,
...
with c1:
    ...
    modo = st.selectbox(
        ...
        index=0,
        help=(
            "Escolha uma única abrangência. Em Todos os Estados, o sistema "
            "percorre as UFs disponíveis para a operação selecionada."
        ),
    )
    todos_os_estados = modo == "Todos os Estados"
with c2:
...
try:
    ...
    indice_rreo_atual = _indice_rreo_por_municipio(arquivos_pdf)
    indice_fnde_atual = (
...
if modo=="Município único" and not todos_os_estados:
    arquivo_selecionado,nota_pdf=localizar_pdf_municipio(municipio_selecionado,arquivos_pdf,indice_rreo_atual)
    ...
    for municipio_item in municipios_selecionados:
        arquivo_item,_=localizar_pdf_municipio(municipio_item,arquivos_pdf,indice_rreo_atual)
...
with left:
    ...
    for mun in lista_preview[:100]:
        arq,nota=localizar_pdf_municipio(mun,arquivos_pdf,indice_rreo_atual)
...
if executar:
    ...
    try:
                    ...
                    trabalhos.append(
                        {
                            ...
                            "indice_rreo": _indice_rreo_por_municipio(arquivos_item),
            ...
            trabalhos.append(
                {
                    ...
                    "uf": uf,
                    # Para RREO, todos os PDFs do estado precisam ser lidos para
                    # descobrir o município pelo conteúdo interno. A seleção abaixo
                    # limita os municípios a preencher, não os arquivos a inspecionar.
                    "arquivos": arquivos_pdf,
        ...
        # Para o RREO, primeiro cria um índice pelo município escrito DENTRO
        # de cada PDF. Só depois associa o arquivo ao município/código IBGE da
        # planilha-base. Essa regra vale para todos os modos de processamento.
        if PROCESSAR_RREO:
            for trabalho in trabalhos:
                status_slot.info(
                    f"Lendo cabeçalhos internos do RREO - {trabalho['uf']} "
                    f"({len(trabalho['arquivos'])} PDF(s))..."
                )
                indice_interno, falhas_indice = construir_indice_rreo_interno(
                    arquivos_pdf=trabalho["arquivos"],
                    municipios=trabalho["municipios"],
                    uf=trabalho["uf"],
                    temp_root=pasta_temporaria,
                    workers=LOT_SETTINGS.rreo_workers,
                )
                trabalho["indice_rreo_interno"] = indice_interno
                for falha in falhas_indice:
                    logs.append(
                        f"{datetime.now().strftime('%H:%M:%S')}  "
                        f"ÍNDICE RREO {trabalho['uf']}: {falha['arquivo']} - {falha['erro']}"
                    )
        else:
            for trabalho in trabalhos:
                trabalho["indice_rreo_interno"] = {}

        fila: list[dict[str, Any]] = []
        for trabalho in trabalhos:
            for municipio_item in trabalho["municipios"]:
                codigo = municipio_item["codigo_ibge"]
                rreo_preprocessado = trabalho.get("indice_rreo_interno", {}).get(codigo)
                arquivo_rreo = rreo_preprocessado.get("arquivo") if rreo_preprocessado else None
                fila.append({
                    "trabalho": trabalho,
                    "uf": trabalho["uf"],
                    ...
                    "rreo_preprocessado": rreo_preprocessado,
                    "nota_rreo": float(rreo_preprocessado.get("confianca_municipio", 0.0)) if rreo_preprocessado else 0.0,
        ...
        for numero_lote, lote in enumerate(chunks(fila_pendente, LOT_SETTINGS.batch_size), start=1):
            ...
            for index, payload in enumerate(lote):
                ...
                origem_municipio_rreo = ""
                confianca_municipio_rreo = 0.0
                ...
                if PROCESSAR_RREO:
                    if rreo_error:
                        ...
                        origem_municipio_rreo = rreo_data.get("origem_municipio", "")
                        confianca_municipio_rreo = float(rreo_data.get("confianca_municipio", 0.0) or 0.0)

                        if municipio_interno is None:
                            erros_municipio.append(
                                "RREO: município do conteúdo interno não identificado com segurança; "
                                "nenhuma linha foi preenchida."
                            )
                        else:
                            municipio_rreo = municipio_interno
                            nome_arquivo_rreo = extrair_nome_arquivo(arquivo_rreo["name"]) if arquivo_rreo else ""
                            if normalizar_texto(nome_arquivo_rreo) != municipio_rreo["nome_normalizado"]:
                                divergencia_nome_rreo = (
                                    f"NOME_ARQUIVO_DIVERGE_DO_CONTEUDO: arquivo={nome_arquivo_rreo}; "
                                    f"conteúdo={municipio_rreo['nome']}; ação=usado conteúdo interno"
                                )
                        ...
                        if rreo_data.get("error") and municipio_interno is None:
                            erros_municipio.append(rreo_data["error"])
                        if arquivo_rreo:
                ...
                if GERAR_LOG_RREO and PROCESSAR_RREO:
                    ...
                    append_rreo_log(workbook, {
                        ...
                        "Município": municipio_rreo["nome"] if municipio_rreo else "",
                        "Código IBGE": municipio_rreo["codigo_ibge"] if municipio_rreo else "",
                        "UF": uf_item, "Ano": ano,
                        "Estado/Lote": trabalho["estado_cloud"],
                        "Linha da planilha": municipio_rreo["row"] if municipio_rreo else "",
                        "Município no nome do arquivo": extrair_nome_arquivo(arquivo_rreo["name"]) if arquivo_rreo else "",
                        "Município no conteúdo": municipio_rreo["nome"] if municipio_rreo else "",
                        ...
                        "Método de extração": "PARALELO_LIMITADO + MUNICIPIO_INTERNO",
                    })
            ...
            for trabalho in trabalhos:
                for municipio in trabalho["municipios"]:
                    if municipio["codigo_ibge"] in processed_ibge:
                        continue
                    arquivo, _ = localizar_pdf_municipio(
                        municipio,
                        trabalho["arquivos"],
