...
from core.indice_rreo_interno import build_rreo_internal_index
...
def _arquivo_rreo_por_indice_interno(
    ...
    if not indice_interno:
        return None, 0.0
    item = indice_interno.get("por_ibge", {}).get(municipio["codigo_ibge"])
    if not item:
        return None, 0.0
    return {
        "name": item.get("name", ""),
...
def carregar_indice_rreo_interno(
    ...
    return build_rreo_internal_index(
        uf=uf, year=year, files=arquivos, municipalities=municipios,
        max_workers=LOT_SETTINGS.rreo_workers,
    )
...
try:
    ...
    indice_rreo_atual = {}
    if PROCESSAR_RREO and arquivos_pdf:
        with st.spinner("Validando municípios pelo conteúdo interno dos PDFs RREO..."):
