...
def _arquivo_rreo_por_indice_interno(
    ...
    """Compatibilidade: consulta índice leve criado pelo nome externo."""
    if not indice_interno:
        return None, 0.0
    chave = f"{normalizar_texto(municipio['nome'])}|{municipio['uf']}"
    item = indice_interno.get("por_nome_uf", {}).get(chave)
    return (item, 1.0) if item else (None, 0.0)
...
def carregar_indice_rreo_interno(
    ...
    """Índice rápido pelo nome externo; não abre nem baixa PDFs."""
    del year, municipios_serializados
    arquivos = [
    ...
    for item in arquivos:
        nome = extrair_nome_arquivo(item.get("name", ""))
        if nome:
            por_nome_uf.setdefault(f"{nome}|{str(uf).upper()}", item)
    return {
        "uf": str(uf).upper(),
...
# Estas duas variáveis são a única autoridade sobre quais módulos serão
# executados. Nenhuma configuração antiga pode ativar o outro fluxo.
PROCESSAR_RREO = operacao_rotulo in {"RREO", "RREO + FNDE"}
PROCESSAR_FNDE = operacao_rotulo in {"FNDE", "RREO + FNDE"}
...
if modo=="Município único" and not todos_os_estados:
    ...
    indice_externo = _indice_rreo_por_municipio(arquivos_pdf)
    for municipio_item in municipios_selecionados:
        arquivo_item,_=localizar_pdf_municipio(municipio_item, arquivos_pdf, indice_externo)
...
with left:
    ...
    indice_preview = _indice_rreo_por_municipio(arquivos_pdf)
    for mun in lista_preview[:100]:
        arq,nota=localizar_pdf_municipio(mun, arquivos_pdf, indice_preview)
        ...
        if todos_os_estados:
            ...
            try:
                for estado_item in estados_cloud:
                    ...
                    indice_interno_item = {
                        "por_nome_uf": {
                            f"{extrair_nome_arquivo(item.get('name', ''))}|{uf_item}": item
                            for item in arquivos_item
                            if extrair_nome_arquivo(item.get("name", ""))
                        },
                        "metodo": "NOME_EXTERNO",
                    }
                    trabalhos.append(
            ...
            if PROCESSAR_RREO and arquivos_pdf and not indice_rreo_atual:
                indice_rreo_atual = {
                    "por_nome_uf": {
                        f"{extrair_nome_arquivo(item.get('name', ''))}|{uf}": item
                        for item in arquivos_pdf
                        if extrair_nome_arquivo(item.get("name", ""))
                    },
                    "metodo": "NOME_EXTERNO",
                }
            ...
            for index, payload in enumerate(lote):
                ...
                if PROCESSAR_RREO:
                    if rreo_error:
                        ...
                        if municipio_interno:
                            municipio_interno_nome = str(municipio_interno.get("nome") or "")
                            origem_municipio_rreo = rreo_data.get("origem_municipio", "CONFERENCIA_LOCAL")
                            confianca_municipio_rreo = float(
                                rreo_data.get("confianca_municipio", 0.0) or 0.0
                            )
                            if (
                                confianca_municipio_rreo >= 0.93
                                and normalizar_texto(municipio_interno_nome)
                                != municipio_encontrado["nome_normalizado"]
                            ):
                                divergencia_confirmada = True
                                nome_arquivo_rreo = (
                                    extrair_nome_arquivo(arquivo_rreo["name"])
                                    if arquivo_rreo else municipio_encontrado["nome"]
                                )
                                divergencia_nome_rreo = (
                                    f"NOME_ARQUIVO_DIVERGE_DO_CONTEUDO: "
                                    f"arquivo={nome_arquivo_rreo}; "
                                    f"conteúdo={municipio_interno_nome}"
                                )
                                acao_divergencia_rreo = (
                                    "PDF_RREO_NAO_PROCESSADO; MUNICIPIO_NAO_TROCADO; "
                                    "LOTE_CONTINUOU"
                                )
                                erros_municipio.append(
                                    "RREO: divergência confirmada entre nome externo e conteúdo; "
                                    "PDF ignorado e lote continuado."
                                )

                        if not divergencia_confirmada:
                ...
                if GERAR_LOG_RREO and PROCESSAR_RREO:
                    ...
                    append_rreo_log(workbook, {
                        ...
                        "Confiança da identificação": f"{confianca_municipio_rreo:.2%}" if municipio_interno_nome else "",
                        "Divergência de município": "SIM" if divergencia_confirmada else "NÃO",
                        ...
                        "Método de extração": "NOME_EXTERNO + EXTRAÇÃO_LOCAL + CONFERÊNCIA_INTERNA_SEM_GEMINI",
                    })
            ...
            for trabalho in trabalhos:
                for municipio in trabalho["municipios"]:
                    if municipio["codigo_ibge"] in processed_ibge:
                        continue
                    indice_externo_pendente = _indice_rreo_por_municipio(
                        trabalho.get("arquivos", [])
                    )
                    arquivo, _ = localizar_pdf_municipio(
                        municipio,
                        trabalho.get("arquivos", []),
                        indice_externo_pendente,
                    )
