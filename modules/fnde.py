...
def _ocr_text(images: list[tuple[int, bytes]]) -> str:
    ...
    for page_number, data in images:
        ...
        text = pytesseract.image_to_string(image, lang=os.getenv("TESSERACT_LANG", "por+eng"), config="--psm 6")
        pages.append(f"\n===== PÁGINA {page_number} =====\n{text}")
        image.close()
    return "\n".join(pages)
...
def extrair_com_gemini(
    ...
    try:
        ...
        if os.getenv("FNDE_USAR_OCR_FALLBACK", "true").lower() not in ("1", "true", "sim", "yes"):
            raise
        method = "OCR_LOCAL_FALLBACK_OK"
        warnings.append(f"Gemini Vision falhou: {type(vision_error).__name__}: {vision_error}")
