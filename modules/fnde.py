...
REGRAS OBRIGATÓRIAS:
1. Cada programa deve ser extraído SOMENTE do seu próprio bloco. Nunca atravesse
   o início de outro programa e nunca associe programas apenas por semelhança.
2. PNAE: aceitar exclusivamente o bloco cujo título contenha
   "PROGRAMA NACIONAL DE ALIMENTAÇÃO ESCOLAR". Usar o "Valor Total" do bloco.
3. PNATE: aceitar exclusivamente o bloco cujo título contenha
   "PROGRAMA NACIONAL DE APOIO AO TRANSPORTE DO ESCOLAR" ou a abreviação textual
   inequívoca desse mesmo título. Usar o "Valor Total" do bloco.
4. PDDE: aceitar exclusivamente o bloco cujo título contenha exatamente
   "PROGRAMA DINHEIRO DIRETO NA ESCOLA". Não incluir "ANTIGO PDDE ESTRUTURA",
   Ensino Médio Inovador, Água e Esgotamento Sanitário, Escola do Campo,
   Escola Acessível, PDE Escola, Mais Cultura, Escola de Fronteira,
   Atleta na Escola, Escola Sustentável nem qualquer outro programa.
5. QSE/QESE: aceitar exclusivamente o bloco "QUOTA ESTADUAL / MUNICIPAL" ou
   uma identificação inequívoca de SALÁRIO-EDUCAÇÃO/QSE/QESE referente à mesma
   transferência. Usar o "Valor Total" do bloco.
6. Quando um bloco tiver várias esferas, usar somente o "Valor Total" exibido
   no título do próprio bloco. A soma das esferas serve apenas para conferência.
   Não somar o total do título novamente com as linhas das esferas.
7. Se houver mais de um bloco verdadeiro do mesmo programa, somar apenas os
   "Valor Total" desses blocos, uma vez por bloco.
8. Para cada bloco incluído, registrar programa, título completo, valor total,
   página e justificativa. Não registrar programas descartados como ocorrência.
9. Converter "1.873.565,86" para 1873565.86.
10. Se um programa não existir, retornar 0.0.
11. Se houver dúvida, registrar em avisos e não inventar valor.
12. Confirmar município e código IBGE pelo nome e pelo conteúdo quando possível.
...
def _classificar_titulo_programa(titulo: str) -> str | None:
    ...
    if "PROGRAMA NACIONAL DE ALIMENTACAO ESCOLAR" in texto:
        return "PNAE"

    if (
        "PROGRAMA NACIONAL DE APOIO AO TRANSPORTE DO ESCOLAR" in texto
        or "PROGRAMA NACIONAL DE APOIO AO TRANSP DO ESCOLAR" in texto
    ):
        return "PNATE"

    if "PROGRAMA DINHEIRO DIRETO NA ESCOLA" in texto:
        return "PDDE"

    if (
        "QUOTA ESTADUAL MUNICIPAL" in texto
        or "SALARIO EDUCACAO" in texto
        or re.search(r"\bQSE\b", texto)
        or re.search(r"\bQESE\b", texto)
    ):
        return "QSE"
